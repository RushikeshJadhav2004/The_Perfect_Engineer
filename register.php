        ...
        try {
            ...
            if ($checkEmail->rowCount() > 0) {
                ...
                if ($role === 'founder') {
                  ...
                  // Insert into developers table (with role column)
                    $sql = "INSERT INTO developers (name, email, password, role) VALUES (:name, :email, :password, :role)";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([
                       ':name' => $name,
                       ':email' => $email,
                       ':password' => $hashedPassword,
                       ':role' => $role // Ensure role is saved as 'developer'
                   ]);

                }

                $_SESSION['message'] = "<div class='success-message'>🎉 Registration successful! <a href='login.php'>Login here</a></div>";
            }
